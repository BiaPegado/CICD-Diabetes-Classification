...
def balance_df():
    ...
    save_table_wb(
        "Data/dataset_balanceado.csv", "diabetes_prediction__table_balanceada"
    )
    return df
...
def set_expectations(df):
    ...
    diabetes_expectation_suite.add_expectation(
        ge.core.ExpectationConfiguration(
            ...
            kwargs={"column": "bmi", "min_value": 9, "max_value": 150},
        )
    )
    ...
    diabetes_expectation_suite.add_expectation(
        ge.core.ExpectationConfiguration(
            ...
            kwargs={"column": "hypertension", "value_set": [0, 1]},
        )
    )

    data_asset = ge.from_pandas(df)
    validation_results = data_asset.validate(
        expectation_suite=diabetes_expectation_suite
    )
